...
import { AiFillApple,AiOutlineShoppingCart,AiOutlineDown } from 'react-icons/ai';
...
const ReviewSchema = Yup.object().shape({
  ...
  let stylee={
    ...
    let subHeaderStyle = {
      fontFamily:'Smooch Sans', fontSize:'2.2rem', fontWeight:700, color:'black',}
    let subHead = {
...
let totalTemp = []
  ...
  return (
    <div style={{width:'100vw',height:'100%', display:'flex',gap:20,overflowX:'hidden', flexDirection:'column', justifyContent:'flex-start', alignItems:'center', background:'white'}}>
        <span style={{...subHeaderStyle}}>KYN MERCH</span>

        <div 
       
           onClick={() => setOpenModal(!openModal)}
        style={{backgroundColor:'rgba(220,220,220,0.2)',borderTopWidth:1,borderTopColor:'rgba(220,220,220,0.7)',borderBottomWidth:1,borderBottomColor:'rgba(220,220,220,0.7)', width:'100vw',height:openModal ? '50vh':60,display:'flex', flexDirection:'column',padding:20,justifyContent:'center',zIndex:30  }}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',height:openModal && '10%',transform:openModal && 'translateY(-10px)',
       
          }}>
                <div style={{display:'flex', width:'75%',alignItems:'center',gap:10}}>
                    <AiOutlineShoppingCart style={{fontSize:'1.8rem', left:openModal ? -10:10,position:'absolute'}} />
                    <span style={{display:'flex',alignItems:'center',fontFamily:'Smooch Sans',fontSize:'1.25rem',fontWeight:500,justifyContent:'center',gap:5,marginLeft:20,
                   
                  
                  }}>Show order summary <AiOutlineDown /> </span>
                  
                </div>
           
                <div>
                <span style={{fontFamily:'Smooch Sans',fontSize:'1.5rem',fontWeight:500}}>${cartItems !== null && cartItems.filter((i,index) => index!==0).map((item) => item.total).reduce((a,b)=>a+b,0)}</span>
                </div>
            </div>
            <div style={{width:'120%',height:1, backgroundColor:'lightgray',marginTop:5,alignSelf:'flex-start',transform:'translateX(-10%)'}}>
            ...
            {openModal && (
                ...
                {cartItems !== null && cartItems.filter((i,index) => index !==0).map((item,index) => (
                    <div style={{display:'flex', width:'100%', margin:'0 auto', height:120,
                    alignItems:'center',
                    gap:25, 
                    transform:'translateY(20px)',
                      ...
                      <div style={{position:'relative'}}>
                        ...
                        <span style={{position:'absolute',right:0,fontFamily:'Smooch Sans',fontWeight:400,fontSize:'1.4rem'}}>
                         ${item.number * 40}
                        </span>
                      </div>
                  ))}
                  
                <div style={{width:'100%',justifyContent:'space-between', display:'flex', alignItems:'center',marginTop:20,}}>
                    <span style={{fontFamily:'Smooch Sans',fontWeight:600,fontSize:'1.3rem',color:'gray'}}>Subtotal</span>
                    <span style={{fontFamily:'Smooch Sans',fontWeight:400,fontSize:'1.4rem'}}>${cartItems !== null && cartItems.filter((i,index) => index!==0).map((item) => item.total).reduce((a,b)=>a+b,0)}</span>
                </div>
                <div style={{width:'100%', display:'flex',justifyContent:'space-between', alignItems:'center'}}>
                    <span style={{fontFamily:'Smooch Sans',fontWeight:600,fontSize:'1.3rem',color:'gray'}}>Shipping</span>
                    <span style={{fontFamily:'Smooch Sans',fontWeight:400,fontSize:'1.4rem'}}>$10</span>
                </div>
                <div style={{width:'100%',height:1, backgroundColor:'lightgray',marginTop:20,alignSelf:'flex-start',transform:'translateX(0%)'}}>

                </div>
                <div style={{width:'100%',justifyContent:'space-between', display:'flex', alignItems:'center',marginTop:20}}>
                     <span style={{fontFamily:'Smooch Sans',fontWeight:600,fontSize:'1.8rem',color:'gray'}}>Total</span>
                     <span style={{fontFamily:'Smooch Sans',fontWeight:400,fontSize:'1.6rem'}}>${cartItems !== null && cartItems.filter((i,index) => index!==0).map((item) => item.total).reduce((a,b)=>a+b,10)}</span>
                </div>
           </> )}

        </div>  
        <div style={{width:'90%',margin:'0 auto',display:'flex',flexDirection:'column',justifyContent:'center',alignItems:'center',
                       ...
                       borderBottomLeftRadius:8,borderBottomRightRadius:8,marginTop:20
      }}>
      ...
      borderWidth:1,borderRadius:5,borderColor:'transparent'}}>
        ...
        <h2 style={{width:'90%',textAlign:'center',lineHeight:'0.1em',margin:'10px 0 20px',borderBottomWidth:1,transform:'translate(0px,-12px)',
        ...
        <div style={{display:'flex', justifyContent:'flex-start',width:'90%',flexDirection:'column',transform:'translateY(-20px)',overflow:'hidden',height:'100vh'}}>
